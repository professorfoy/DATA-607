...
opponent_list
```
#### There are some empty cells that do not contain the results of a game.  For example, ASHWIN BALAJI appears as list number 62 and only played in the first round, but did not play in the subsequent rounds. To fill the gaps so that all the rows have entries, we record a 0 for each empty cell.
...
#### We used the "for loop" function to tell R to look at the data points from the first to the last in the list of Opponents, which is located in the opponent_list2 object.  Then...
...
for (i in 1:length(opponent_list2)) {
  
  # Iterate through every opponent_list2 vector
  for (s in i) {
    
    # Initialize a running sum and number of matches
    sum = 0
    number_of_matches = 0

    # Cast the index as an integer
    opponent_indexes = as.integer(opponent_list2[[s]])
    
    # Iterate through every opponent index
    for (oi in opponent_indexes){
      # We only increment the running sum and number_of_matches if there is a non-zero opponent index
      if (oi != 0) {
        sum = sum + as.integer(Player_PreRating[[oi]])
        number_of_matches = number_of_matches + 1
      }
    }
    
    # Calculate the average pre rating and add to the number vector
    Average_Prechess_Rating_Opponent[i] <- round(sum / number_of_matches)
  }
}
